...
Generator::Generator(int locality_range, double locality_shift, int size, int size_factor)
{
...
std::tuple<key_type, Cache::val_type, std::string> Generator::gen_req(bool print_results)
{
    ...
    int current = 0;
    int i = current_locality_*(locality_shift_*(data_.size()-1))/locality_range_;//we start some ways into the data vector
    data_type kv_tuple;
